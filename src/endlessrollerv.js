...
function showGameOverMessage() {
  ...
  msg.style.border = '4px solid #ff4444';
  msg.style.borderRadius = '15px';
  msg.style.textAlign = 'center';
  msg.innerHTML = "Has chocado con cinco pinos, perdiste.";
...
function doTreeLogic(){
	...
	treesInPath.forEach( function ( element, index ) {
		...
		if(treePos.z>6 &&oneTree.visible){
			...
			if(treePos.distanceTo(heroSphere.position)<=0.6 && element.visible){
                console.log("hit");
                hasCollided=true;
                explode();

                // Incrementar colisiones y verificar derrota
                collisionCount++;
                if (collisionCount >= maxCollisions) {
                    juegoTerminado = true;  // Detener animación
                    showGameOverMessage();
                    return;
                }
            }
...
function actualizarHUD() {
    if (hud) {
        hud.innerHTML = `🏁 Pinos esquivados: ${avoidedCount}`;
    }
}
